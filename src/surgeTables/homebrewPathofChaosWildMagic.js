const homebrewPathofChaosWildMagic = [
  "You explode, casting Fireball as a 3rd-level spell centered on yourself. You are set on fire as a result of this explosion and take 1d6 Fire damage at the start of each of you turns and your weapon attacks deal fire damage until your Rage ends.",
  "Acid spews from your pores.  Each creature in that area must make a Dexterity saving throw, taking 2d8 acid damage on a failed save, or half as much damage on a successful one. Until your rage ends, you can use this effect again on each of your turns as a bonus action.",
  "Your gravitational pull shifts. Each creature within 40 feet of you must make a Strength saving throw, or be pulled 10 feet toward you.",
  "Shadowy tendrils lash around you. Each creature of your choice that you can see within 30 feet of you must succeed on a Constitution saving throw or take 1d12 necrotic damage. You also gain 1d12 temporary hit points.",
  "You teleport up to 30 feet to an unoccupied space you can see. Until your rage ends, you can use this effect again on each of your turns as a bonus action.",
  "An intangible spirit, which looks like a flumph or a pixie (your choice), appears within 5 feet of one creature of your choice that you can see within 30 feet of you. At the end of the current turn, the spirit explodes, and each creature within 5 feet of it must succeed on a Dexterity saving throw or take 1d6 force damage. Until your rage ends, you can use this effect again, summoning another spirit, on each of your turns as a bonus action.",
  "Magic infuses one weapon of your choice that you are holding. Until your rage ends, the weapon’s damage type changes to force, and it gains the light and thrown properties, with a normal range of 20 feet and a long range of 60 feet. If the weapon leaves your hand, the weapon reappears in your hand at the end of the current turn.",
  "Whenever a creature hits you with an attack roll before your rage ends, that creature takes 1d6 force damage, as magic lashes out in retribution.",
  "Until your rage ends, you are surrounded by multi­colored, protective lights; you gain a +1 bonus to AC, and while within 10 feet of you, your allies gain the same bonus.",
  "Flowers and vines temporarily grow around you; until your rage ends, the ground within 15 feet of you is difficult terrain for your enemies.",
  "A bolt of light shoots from your chest. Another creature of your choice that you can see within 30 feet of you must succeed on a Constitution saving throw or take 1d6 radiant damage and be blinded until the start of your next turn. Until your rage ends, you can use this effect again on each of your turns as a bonus action.",
  "Cold emanates from your weapon, chilling the air. Until your rage ends, when you hit a creature with a melee weapon attack, it must succeed on a Constitution saving throw or have disadvantage on the next weapon attack roll it makes before the end of its next turn. A creature killed by your attacks becomes a frozen statue until it thaws.",
  "Your rage grants you perfect clarity. Until your rage ends, you have truesight out to 120 feet.",
  "Your weapon is infused with the power to break the boundaries between planes of existence. Until your rage ends, when you hit a creature with a weapon attack, they must succeed on a Charisma saving throw or be banished to the Ethereal Plane until the start of your next turn.",
  "Your voice becomes a thunderous roar. Each creature in a 15-foot cone originating from you must make a Constitution saving throw. On a failed save, a creature takes 2d8 thunder damage and is pushed 10 feet away from you. On a successful save, the creature takes half as much damage and isn't pushed. Until your rage ends, you can use this effect again on each of your turns as a bonus action.",
  "Rose petals swirl around you. One creature of your choice must succeed on a Wisdom saving throw, or take 1d10 psychic damage and be charmed by you until the start of your next turn.",
  "A protective sheen of magical energy covers you. Until your rage ends, you have resistance to fire, cold, acid, lightning, thunder, poison, radiant, and necrotic damage.",
  "You feel your body growing rapidly. Your size increases by one size category until your rage ends. While enlarged in this manner, your weapon attacks deal an extra 1d6 damage.",
  "The world seems to slow down around you. Until your rage ends, your movement speed increases by 10 feet, and you can Dash, Disengage, Hide, or Use an Object as a bonus action.",
  "Spectral wings emanate from your back. Until your rage ends, you can a fly speed of 30 feet. If you already have a fly speed, it is increased by 30 feet."
]

export default homebrewPathofChaosWildMagic;
